...
pdb_code = "1brfA"
pdb_filename = "./data/pdb/1brfA.pdb"
gap = 4
...
def main():
    ...
    df = pd.read_csv("./data/corr/corr_1brfA.csv")
    corr = df.values[:, 1:]
    #heatmap(np.abs(corr))
    model = graphical_lasso(corr, alpha=.05, tol=1e-8, enet_tol=1e-5, max_iter=500)
    prec = model[1]
    ...
    #print(cpr)
    print(top_k_acc(contact_map, est_contact_map, k=p/4, gap=gap))
    print(top_k_acc(contact_map, np.abs(corr), k=p/4, gap=gap))
