...
pdb_code = "1brfA"
pdb_filename = "./data/pdb/1m8aA.pdb"
gap = 4
...
def main():
    ...
    df = pd.read_csv("./data/corr/corr_1m8aA.csv")
    corr = df.values[:, 1:]
    #heatmap(np.abs(corr))
    model = graphical_lasso(corr, alpha=.05, tol=1e-8, enet_tol=1e-5, max_iter=200)
    prec = model[1]
    ...
    #print(cpr)
    print(top_k_acc(contact_map, est_contact_map, k=p/5, gap=gap))
    print(top_k_acc(contact_map, np.abs(corr), k=p/5, gap=gap))
